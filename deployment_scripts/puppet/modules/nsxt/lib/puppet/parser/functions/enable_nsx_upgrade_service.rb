...
module Puppet::Parser::Functions
  newfunction(:enable_nsx_upgrade_service, :type => :rvalue, :doc => <<-EOS
Returns the address of nsx-t manager, on which enable install-upgrade service
example:
  enable_nsx_upgrade_service('172.16.0.1,172.16.0.2,172.16.0.3', username, password)
EOS
  ) do |args|
    managers = args[0]
    username = args[1]
    ...
    managers.split(',').each do |manager|
      ...
      if check_service_running(service_enabled_on_manager, username, password)
        return get_manifest_url(service_enabled_on_manager, username, password)
      else
...
def get_manifest_url(manager, username, password)
  node_version = get_node_version(manager, username, password)
  manifest = "http://#{manager}:8080/repository/#{node_version}/metadata/manifest"
end
