...
CheckVariables() {
  ...
  if [ -z "${WORKSTATION_IFS}" ]; then
    export WORKSTATION_IFS="vmnet1 vmnet2 vmnet3 vmnet5"
  fi
  ...
  if [ -z "${NSXT_TIER0_ROUTER_UUID}" ]; then
    export NSXT_TIER0_ROUTER_UUID='ed9d5f1d-8524-4ded-9cdc-59eb2a008bff'
  fi
  if [ -z "${NSXT_EDGE_CLUSTER_UUID}" ]; then
    export NSXT_EDGE_CLUSTER_UUID='0467ef56-7037-11e6-b26b-005056ab1601'
  fi
  ...
  if [ -z "${NSXT_FLOATING_NET_GW}" ]; then
    export NSXT_FLOATING_NET_GW=''
  fi
  if [ -z "${NSXT_INTERNAL_NET_CIDR}" ]; then
    export NSXT_INTERNAL_NET_CIDR='192.168.250.0/24'
  fi
...
setup_management_net() {
  ...
  floating_ip=${NSXT_FLOATING_NET_GW}
  floating_mask=${NSXT_FLOATING_NET_CIDR##*\/}
