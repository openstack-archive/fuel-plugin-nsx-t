...
class TestNSXtFailover(TestNSXtBase):
    ...
    @test(depends_on=[TestNSXtSystem.nsxt_ha_mode],
    ...
    def nsxt_shutdown_controller(self):
        ...
        Scenario:
            1. Revert to deployed env with plugin.
               Nodes: 3 controllers, 1 compute.
            2. Get access to OpenStack.
            3. Create vcenter VM and check connectivity to outside world
               from VM.
            4. Shutdown primary controller.
            5. Ensure that VIPs are moved to another controller.
            6. Ensure that there is a connectivity to outside world from
               created VM.
            7. Create new network and attach it to default router.
            8. Create vCenter VM with new network and check network
               connectivity via ICMP.

        Duration: 180 min
        """
        # Revert to deployed env with plugin. Nodes: 3 controllers, 1 compute
        self.show_step(1)
        ...
        ips = [os_conn.get_nova_instance_ip(server,
                                            net_name=self.default.PRIVATE_NET)
               for server in servers]

        private_net = os_conn.nova.networks.find(
        ...
        _, access_point_ip = os_help.create_access_point(
            ...
            security_groups=[ssh_sg.name, default_sg['name']])

        ip_pairs = {ip: ['8.8.8.8'] for ip in ips}
        os_help.check_connection_through_host(access_point_ip, ip_pairs)

        self.show_step(4)  # Shutdown primary controller
        primary_ctrl_devops = self.fuel_web.get_nailgun_primary_node(
            self.env.d_env.nodes().slaves[0])
        self.fuel_web.warm_shutdown_nodes([primary_ctrl_devops])

        self.show_step(5)  # Ensure that VIPs are moved to another controller
        ...
        self.show_step(7)  # Create new network and attach it to default router
        net_1 = os_conn.create_network(network_name='net_1')['network']
        os_conn.create_subnet(subnet_name='subnet_1',
                              network_id=net_1['id'],
        ...
        self.show_step(8)
        os_help.create_instance(os_conn, az='vcenter', net=net_1)

        servers = os_conn.get_servers()
        ips_1 = [os_conn.get_nova_instance_ip(server, net_name='net_1')
                 for server in servers]

        _, access_point_ip = os_help.create_access_point(
            os_conn=os_conn,
            nics=[{'net-id': private_net.id}, {'net-id': net_1.id}],
            security_groups=[ssh_sg.name, default_sg['name']])

        ip_pairs = {key: ips_1 for key in ip_pairs}
        os_help.check_connection_through_host(access_point_ip, ip_pairs)
    ...
    def nsxt_reboot_vcenter(self):
        ...
        Scenario:
            ...
            9. Ensure that all instances from vCenter are displayed on
               dashboard.
            10. Ensure there is a connectivity between VMs.
            11. Run OSTF.
        ...
        self.show_step(3)  # Launch VM_1 with image TestVM-VMDK and vcenter az
        os_help.create_instance(os_conn, az='vcenter')

        self.show_step(4)  # Launch VM_2 with image TestVM-VMDK and vcenter az
        os_help.create_instance(os_conn, az='vcenter')

        self.show_step(5)  # Check connection between VM_1 and VM_2
        servers = os_conn.get_servers()
        ips = [os_conn.get_nova_instance_ip(server,
                                            net_name=self.default.PRIVATE_NET)
               for server in servers]
        ...
        self.show_step(6)  # Reboot vcenter
        vcenter_name = [name for name in WORKSTATION_NODES
                        if 'vcenter' in name].pop()
        host_type = 'ws-shared'
        path_to_vmx_file = '"[standard] {0}/{0}.vmx"'.format(vcenter_name)
        ...
        self.show_step(7)  # Check that controller lost connection with vCenter
        wait(lambda: not icmp_ping(VCENTER_IP),
             interval=1,
             timeout=10,
             timeout_msg='vCenter is still available.')

        self.show_step(8)  # Wait for vCenter is online
        wait(lambda: icmp_ping(VCENTER_IP),
             interval=5,
             timeout=120,
             timeout_msg='vCenter is not available.')

        # Ensure that all instances from vCenter are displayed on dashboard
        self.show_step(9)
        os_help.check_instances_state(os_conn)

        self.show_step(10)  # Ensure there is a connectivity between VMs
        os_help.ping_each_other(ips=ips, access_point_ip=access_point_ip)

        self.show_step(11)  # Run OSTF
