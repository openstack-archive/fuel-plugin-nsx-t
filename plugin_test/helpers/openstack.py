...
def create_instance(os_conn, net=None, az='nova', sg_names=['default']):
...
def check_instances_state(self, os_conn):
...
def check_service(ip, commands):
        ...
        for cmd in commands:
            wait(
                lambda:
                ':-)' in ssh_manager.check_call(ip=ip,
                                                command=cmd).stdout[-1],
                timeout=200)
