...
class HOpenStack(ShowPos):
    ...
    def __init__(self, nsxt_ip, user=None, password=None, tenant=None):
        """Init Common.

        :param nsxt_ip: controller ip
        """
        ...
        self._common = Common(controller_ip=nsxt_ip,
                              user=user,
                              password=password,
                              tenant=tenant)
    ...
    def create_network(self, name):
        ...
        logger.debug("Created network with id '{0}'".format(network_id))

        return network['network']

    def create_subnetwork(self, network, cidr, ipv=4):
        """Create a subnet.
        ...
        subnet_params = {
            "subnet": {
                ...
                "ip_version": ipv,
                "cidr": cidr,
                "name": 'subnet_{}'.format(network['name']),
            }
        }
        subnet = self._common.neutron.create_subnet(subnet_params)['subnet']
        logger.debug("Created sub network with id '{0}'".format(subnet['id']))
    ...
    def aggregate_create(self, name, availability_zone):
        ...
        :return: response on create aggregation request with details
        """
    ...
    def aggregate_host_remove(self, aggregate, hostname):
        ...
        :return: response on remove_host request with details
        """
    ...
    def aggregate_host_add(self, aggregate, hostname):
        ...
        :param hostname: host name
        :return: response on add_host request with details
        """
    ...
    def role_get(self, role_name):
        ...
        try:
            ...
            logger.warning('User with role {0} was not found.'.format(role_name))
            return None
        return role
    ...
    @get_openstack_list_paginator()  # use pagination with default page size
    ...
    def tenant_assign_user_role(self, tenant, user, role):
        """Link tenant with user and role.
    ...
    def security_group_add_rule(self, tenant_id, group):
        ...
        try:
            ...
            logger.warning(
                "Can't create rule for the tenant {0}. Exception: {1}".format(
                    tenant_id, e))
