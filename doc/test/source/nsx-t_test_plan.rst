================================
Test Plan for NSX plugin v1.0.0
================================
...
Fuel NSX plugin includes NSX plugin for Neutron which is developed by
third party. This test plan covers a full functionality of Fuel NSX plugin,
...
* VMware NSX plugin can be enabled only with Neutron tunnel segmentation.
* Environment with enabled VMware NSX plugin can't contains compute nodes.
...
* Install/uninstall Fuel NSX plugin
* Deploy Cluster with Fuel NSX plugin by Fuel
    * Roles of nodes
        * controller
        * mongo
        * compute-vmware
...
VMware NSX plugin work after new build delivery. Smoke tests will be used by
...
components of Fuel and MOS work effectively with Fuel VMware NSX plugin
without gaps in data flow.

**Regression testing**

The goal of regression testing is to verify that key features of Fuel VMware
NSX plugin are not affected by any changes performed during preparation to
...
The goal of acceptance testing is to ensure that Fuel VMware NSX plugin has
